...
![호스트 별 다중 서비스 인스턴스](/assets/images/post/book/microservice-to-study-with-picture/multiple-service-instance-per-host.png)
  |장점|단점|
  |---|---|
...
![호스트 별 단일 서비스 인스턴스](/assets/images/post/book/microservice-to-study-with-picture/single-service-instance-per-host.png)
  |장점|단점|
  |---|---|
...
- CSP에서 제공하는 배포 파이프라인을 통해 필요한 소프트웨어가 패키지화 된 가상 머신 이미지를 서비스 인스턴스트로 제공 → 서비스 수가 적은 경우 컨테이너 형 가상화보다 적은 비용으로 배포 할 수도 있음
  ...
  |장점|단점|
  |---|---|
  |<li>서비스 인스턴스를 패키지화 할 수 있다.</li><li>서비스 인스턴스를 분리할 수 있다.</li>|<li>배포에 시간이 걸린다</li><li>리소스 사용법이 비효율적이다.</li><li>시스템 관리 비용이 높아진다.</li>|

[컨테이너 별 서비스 인스턴스]
- 컨테이너라는 OS수준의 가상화 기술을 사용해 서비스 인스턴스를 구성하는 패턴
- 컨테이너 이미지 형태(사전에 패키지화 한 서비스)로 배포
- Kubernetes를 이용해서 마이크로서비스를 구성하는 대량의 컨테이너를 효율적으로 관리할 수 있다.

  |장점|단점|
  |---|---|
  |<li>배포 속도가 빠르다.</li><li>리소스 효율이 높다.</li><li>이동성이 좋다.</li>|<li>컨네이너 기반의 구축 및 운영이 쉽지 않다.</li>|

[서버리스 배포]
- Public Cloud가 제공하는 서버리스 서비스를 사용해 서비스를 배포하는 패턴
- HTTP를 사용하는 웹앱/모바일앱에서 이벤트나 호출을 받으면, 서비ㅓ리스 서비스를 사용해서 애플리케이션 로직을 실행하는 방식
- Public Cloud가 자동확장, 가용성 관리, 유지/보수 등을 알아서 관리해 줌

  |장점|단점|
  |---|---|
  |<li>서버 리소스를 의식하지 않으므로 서버 관리 및 운영이 불필요하다.</li><li>서비스 부하에 따라 자동으로 확장/축소된다.</li><li>서비스 요청량을 기준으로 과금된다..</li>|<li>지연이 발생한다..</li><li>이벤트/요청 기반의 프로그래밍 모델로 제한된다.</li>|

### 4.8 관찰 가능성 패턴
#### 4.8.1. 관찰 가능성 패턴의 배경과 동기
- 장애를 대비해서 감시와 통지, 대응책을 마련하는 것인 IT시스템의 필수 요건임
- 특히 마이크로서비스에서는 감시 대상인 각 서비스나 관련 컴포넌트가 논리적/물리적으로 분산 배치되어 있으므로, 빠짐없이 확실하게 감시하기 위해 요구되는 것이 "관찰 가능성(Observability, 가시성)" 패턴이 필요함
